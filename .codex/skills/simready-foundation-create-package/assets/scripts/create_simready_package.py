...
from sr_pkg_sample._utils import local_path_from_url
...
def _validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    """Cross-check parsed args and report invalid combinations via parser.error."""
...
async def _run_wrapp(args: argparse.Namespace) -> None:
    ...
    if not args.skip_post_validation:
        pkg_def = local_path_from_url(created.pkg_def_url)
        post_result: PostValidationResult | None = None
        try:
