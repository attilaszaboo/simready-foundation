...
# ``<skill>/assets/scripts/.venv`` (i.e. right next to this script) and installs
# the packages listed in ``requirements-package-sample.txt``.  When local wheel paths are
...
step "Installing dependencies from requirements-package-sample.txt"
"${PIP_INSTALL[@]}" -r "$SCRIPT_DIR/requirements-package-sample.txt"
...
Next steps:
  ...
  python create_simready_package.py --help
EOF
